...
# =============================================================================
$AMBIGUOUS        = $false
$AMBIGUITY_REASONS = [System.Collections.Generic.List[string]]::new()

# --- 지시 대명사 + 구체적 대상 없음 ---
if ($MSG_LOWER -match '이거|저거|그거|이것|저것|그것|여기|저기|그쪽|요거') {
    if ($MSG_LOWER -match '해줘|수정|바꿔|고쳐|삭제|지워|만들어|추가|넣어|바꿔줘') {
        $hasFilePath = $USER_MSG -match '[a-zA-Z0-9_/\\.-]{3,}\.(py|js|ts|md|json|ps1|sh|txt|yaml|yml|sql|cpp|h|c)'
        $hasName     = $USER_MSG -match '함수|클래스|변수|메서드|function|class|def |섹션|항목'
        if (-not $hasFilePath -and -not $hasName) {
            $AMBIGUOUS = $true
            $AMBIGUITY_REASONS.Add("지시 대상 불명확 — 파일명·함수명·변수명을 구체적으로 알려주세요")
        }
    }
}

# --- 지시가 너무 짧음 (3단어 이하 + 동작 동사) ---
if ($WORD_COUNT -le 3 -and $MSG_LOWER -match '해줘|수정|고쳐|만들어|추가|삭제|바꿔|넣어|지워') {
    $AMBIGUOUS = $true
    $AMBIGUITY_REASONS.Add("지시가 너무 짧습니다 — 무엇을, 어디서, 어떻게를 추가해주세요")
}

# --- 광범위한 수정 범위 ---
if ($MSG_LOWER -match '전부.*수정|다\s.*바꿔|모든.*고쳐|전체.*변경|싹.*바꿔|다\s.*고쳐') {
    $AMBIGUOUS = $true
    $AMBIGUITY_REASONS.Add("'전체/전부' 범위가 불명확 — 어떤 파일·모듈·기능인지 구체화해주세요")
}
...
# 9. 출력
# =============================================================================
# 모호성 경고 (카테고리 출력 전에 먼저)
if ($AMBIGUOUS) {
    Write-Output ""
    Write-Output "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    Write-Output "⚠ [모호한 지시 감지] 작업 전에 먼저 확인이 필요합니다:"
    foreach ($r in $AMBIGUITY_REASONS) {
        Write-Output "  · $r"
    }
    Write-Output "→ 바로 작업을 시작하지 말고, 위 항목을 사용자에게 먼저 질문하세요."
    Write-Output "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    Write-Output ""
}
...
# =============================================================================
Write-Output ""
Write-Output "[Claude 행동 원칙]"
Write-Output "  · 지시가 모호하면 → 추측해서 진행하지 말고 먼저 질문하세요"
Write-Output "  · 사용자 전제가 틀렸으면 → 작업 전에 먼저 설명하고 확인을 받으세요"
Write-Output "  · 더 나은 방법이 있으면 → 제안하고 선택권을 주세요"
